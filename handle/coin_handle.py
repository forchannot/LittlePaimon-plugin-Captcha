...
class MihoyoBBSCoin:
            ...
            while w == 0 and text < 3:
                ...
                if data["retcode"] != 0:
                    if data["retcode"] == 1034:
                        if self.sign_allow:
                            logger.warning(f"社区签到触发验证码,重试第{w+1}次")
                            text += 1
                    ...
                    logger.info("米游币自动获取", f"➤➤<r>{self.state}</r>")
                    if data["retcode"] != 1034 and text == 3:
                        return f"讨论区签到：{self.state}"
                    if data["retcode"] == 1034 and text == 3:
                        return "讨论区签到：失败，遇到验证码"
                else:
                    w = 1
    ...
    async def read_posts(self):
        ...
        for i in range(self.Task_do["bbs_Read_posts_num"]):
            req = await aiorequests.get(
                url=BBS_DETAIL_URL.format(self.postsList[i][0]), headers=self.headers
            )
            data = req.json()
            if data["message"] == "OK":
                num_ok += 1
    ...
    async def share_post(self):
        ...
        for _ in range(3):
            req = await aiorequests.get(
                url=BBS_SHARE_URL.format(self.postsList[0][0]), headers=self.headers
            )
            data = req.json()
            if data["message"] == "OK":
                return "分享帖子：完成！"
...
async def mhy_bbs_coin(user_id: str, uid: str, sign_allow: bool) -> str:
    """
    执行米游币获取任务
    :param user_id: 用户id
    :param uid: 原神uid
    ...
    for user_id, result_list in coin_result_private.items():
        for result in result_list:
            try:
                ...
                logger.info(
                    "米游币自动获取", "➤➤", {"用户": user_id}, f"发送米游币自动结果失败: {e}", False
                )
            await asyncio.sleep(random.randint(3, 6))
